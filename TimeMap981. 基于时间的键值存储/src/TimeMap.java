...
class TimeMap {
    ...
    HashMap<String, TreeMap<Integer, String>> map2;

    public void set2(String key, String value, int timestamp) {
        map2.computeIfAbsent(key, k-> new TreeMap()).put(timestamp, value);
    }
    ...
    public String get2(String key, int timestamp) {
        ...
        Integer time = map2.get(key).floorKey(timestamp);
        if(time == null) {
            return "";
        }
        return map2.get(key).get(time);
    }
}
